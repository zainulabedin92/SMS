...
class WLSM_Staff_General {
				...
				if ( esc_sql( $_POST['search']['value'] ) ) {
					...
					if ( '' !== $search_value ) {
						...
						$condition .= $wpdb->prepare(
							'(sr.name LIKE %s) OR ' .
							'(sr.admission_number LIKE %s) OR ' .
							'(sr.enrollment_number LIKE %s) OR ' .
							'(sr.phone LIKE %s) OR ' .
							'(sr.email LIKE %s) OR ' .
							'(sr.father_name LIKE %s) OR ' .
							'(sr.father_phone LIKE %s) OR ' .
							'(u.user_email LIKE %s) OR ' .
							'(u.user_login LIKE %s) OR ' .
							'(c.label LIKE %s) OR ' .
							'(se.label LIKE %s) OR ' .
							'(sr.roll_number LIKE %s)',
							$search_value,
							$search_value,
							$search_value,
							$search_value,
	...
	public static function fetch_staff_records( $current_user, $role ) {
		...
		if ( esc_sql( $_POST['search']['value'] ) ) {
			...
			if ( '' !== $search_value ) {
				...
				if ( isset( $is_active ) ) {
					$condition .= $wpdb->prepare(' OR (a.is_active = %d)', $is_active);
				}
				...
				if ($joining_date && isset($format_joining_date)) {
					...
					$condition .= $wpdb->prepare(' OR (a.joining_date LIKE %s)', '%' . $joining_date . '%');
				}
	...
	public static function fetch_roles() {
		...
		$query = $wpdb->prepare( WLSM_M_Staff_General::fetch_role_query( $school_id ) );
		...
		if ( esc_sql( $_POST['search']['value'] ) ) {
			...
			if ( '' !== $search_value ) {
				...
				$condition .= $wpdb->prepare( '(%s LIKE %s)', 'r.name', $search_value_like );
	...
	public static function fetch_inquiries() {
		...
		if ( esc_sql( $_POST['search']['value'] ) ) {
			...
			if ( '' !== $search_value ) {
				...
				if ( isset( $is_active ) ) {
					$condition .= $wpdb->prepare(' OR (iq.is_active = %d)', $is_active);
				}
