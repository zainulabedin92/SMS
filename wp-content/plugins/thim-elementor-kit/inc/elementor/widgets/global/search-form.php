...
class Thim_Ekit_Widget_Search_Form extends Widget_Base {
	...
	public function render() {
		...
		if ( $settings['layout'] == 'modal' ) {
			...
			if ( $settings['skin'] == 'toggle' ) {
				...
				echo '<a class="modalbutton" href="#ekits-search-' . $this->get_id() . '">';
			}
		...
		?>
	<div class="thim-ekits-search-form <?php
	echo $class_layout; ?>"
		 id="ekits-search-<?php
		 echo $this->get_id(); ?>">
		<?php
	...
	<?php
		...
		echo esc_url( home_url( '/' ) ); ?>"
			  method="get" <?php
		Utils::print_unescaped_internal_string( $this->get_render_attribute_string( 'search_form' ) ); ?>>
			<div class="input-group">
				<input type="text" placeholder="<?php
				echo esc_attr( $settings['placeholder_text'] ); ?>"
					   name="s"
				...
				if ( $settings['search_type'] != 'all' ) : ?>
					<input type="hidden" name="post_type"
						   value="<?php
						   echo esc_attr( $settings['search_type'] ); ?>"/>
				<?php
				endif; ?>

				<button type="submit" class="button-search">
					<?php
					Icons_Manager::render_icon( $settings['icon_button_search'], array( 'aria-hidden' => 'true' ) ); ?>

					<?php
					if ( ! empty( $settings['text_button_search'] ) && isset( $settings['text_button_search'] ) ) : ?>
						<span class="text"><?php
							echo esc_html( $settings['text_button_search'] ); ?></span>
					<?php
					endif; ?>
				</button>
			</div>
