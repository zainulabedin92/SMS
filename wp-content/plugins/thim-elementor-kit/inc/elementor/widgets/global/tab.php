...
class Thim_Ekit_Widget_Tab extends Widget_Base {
	...
	protected function render() {
		...
		<div <?php
		echo $this->get_render_attribute_string( 'thim_tab_wrapper' ); ?>>
			<ul class="thim-tabs-nav" role="tablist">
				...
				foreach ( $settings['tabs'] as $index => $tab ) :
					...
					?>
					<li <?php
					$this->print_render_attribute_string( $tab_title_setting_key ); ?>>
						<?php
			...
			<div class="thim-tabs-content">
				...
				foreach ( $settings['tabs'] as $index => $tab ) :
					...
					?>
					<div <?php
					$this->print_render_attribute_string( $tab_title_setting_key_mobile ); ?>>
						<?php
						if ( empty( $settings['tab_title_icon'] ) ) {
							Icons_Manager::render_icon( $tab['tab_title_icon'] );
						}
					...
					<div id="<?php
					echo esc_attr( $prefix_tab_id . $tab_count ); ?>" role="tabpanel"
						 aria-labelledby="<?php
						 echo esc_attr( $prefix_tab_id . $tab_count ); ?>"
						 tabindex="<?php
						 echo esc_attr( 1 === $tab_count ? '0' : '-1' ); ?>" <?php
					echo esc_attr( 1 != $tab_count ? 'hidden' : '' ); ?>>
						<?php
