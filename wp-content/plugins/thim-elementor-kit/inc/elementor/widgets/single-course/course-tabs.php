...
class Thim_Ekit_Widget_Course_Tabs extends Widget_Base {
			...
			$repeater_header->add_control(
				...
				[
					...
					'default'     => '<a href="{{instructor_url}}">{{instructor_display_name}}</a>
										<div class="d-flex align-center gap-30">{{instructor_avatar}}
	...
	public function render_content_tab( $settings, $tab_default ) {
				...
				foreach ( $tabs as $key => $tab ) : ?>
					<div class="tab-item" role="tab"
						 ...
						 echo esc_attr( $active_tab === $key ? 0 : - 1 ); ?>">
							<span
								class="ekits-course-tabs__icon"><?php
								Icons_Manager::render_icon( $tab['icon'] ); ?></span>
						<?php
						echo esc_html( $tab['title'] ); ?>
					</div>
		...
		<div class="ekits-content-course-<?php
		...
		echo $role; ?>>
			...
			<?php
			$this->add_render_attribute( 'content_tab_' . $key, [ 'class' => 'ekits-content-course-tabs' ] ); ?>

			<div class="<?php
			echo esc_attr( $settings['layout'] ); ?>-section ekits-course-tabs">
				<div <?php
				$this->print_render_attribute_string( 'tab_' . $key ); ?> >
								<span class="ekits-course-tabs__icon">
									<?php
									Icons_Manager::render_icon( $tab['icon'] ); ?>
								</span>
					<?php
					echo esc_html( $tab['title'] ); ?>
				</div>
				...
				<div <?php
				$this->print_render_attribute_string( 'content_tab_' . $key ); ?> <?php
				echo esc_attr( $hide ); ?>>
					<?php
